...
case "${1:-}" in
    ...
    "") ;;
    *) echo "Usage: bash scripts/build-macos-app.sh [--with-widget]" >&2; exit 1 ;;
esac
...
if [[ "$with_widget" == true ]]; then
    swift_flags+=(-D WIDGET_ENABLED -target "$(uname -m)-apple-macosx14.0")
fi
...
if [[ "$with_widget" == true ]]; then
    ...
    codesign --force --sign "$SIGNING_IDENTITY" "$app/Contents/MacOS/ai-usage"
    ...
    codesign --force --sign "$SIGNING_IDENTITY" --entitlements "$build_stage/widget.entitlements" "$widget"
    codesign --force --sign "$SIGNING_IDENTITY" --entitlements "$build_stage/app.entitlements" "$app"
else
    codesign --force --sign - "$app/Contents/MacOS/ai-usage"
